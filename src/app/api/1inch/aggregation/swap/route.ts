...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('🔥 1inch Aggregation API - Swap request');
    console.log('📋 Query params:', Object.fromEntries(searchParams.entries()));

    // Required parameters for 1inch Aggregation API swap
    ...
    const apiParams = new URLSearchParams({
      ...
      amount,
      from,
      slippage: searchParams.get('slippage') || '1',
      ...
      includeGas: 'true',
      disableEstimate: searchParams.get('disableEstimate') || 'false',
      allowPartialFill: searchParams.get('allowPartialFill') || 'false'
    });

    const endpoint = `${AGGREGATION_API_CONFIG.baseUrl}/${chainId}/swap?${apiParams}`;
    console.log('🎯 1inch Aggregation API swap endpoint:', endpoint);
    ...
    console.log('✅ 1inch Aggregation API Success - Swap data received');
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🔥 1inch Aggregation API - Swap request (POST)');
    ...
    const apiParams = new URLSearchParams({
      ...
      amount: body.amount,
      from: body.from || body.walletAddress,
      ...
      allowPartialFill: body.allowPartialFill || 'false'
    });
